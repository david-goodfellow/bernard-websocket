            ...
            try:
                ...
                if "type" in data:
                    if data["type"] == "audio":
                        ...
                        await session.active_genai_session.send(
...
async def handle_gemini_responses(websocket: Any, session: SessionState) -> None:
    ...
    try:
        while True:
            async for response in session.active_genai_session.receive():
                try:
                    # Replace audio data with placeholder in debug output
    ...
    while True:
        ...
        try:
            ...
            for function_call in tool_call.function_calls:
                ...
                tool_result = await execute_tool(function_call.name, function_call.args, session.mcp_session)
            ...
            if function_responses:
                ...
                await session.active_genai_session.send(input=tool_response)
        except Exception as e:
...
async def safe_send(websocket: Any, message: dict) -> None:
    """Safely send a JSON message over websocket with validation"""
    try:
        ...
        json.loads(json_message)  # This will raise an exception if invalid
...
async def handle_client(websocket: Any, shared_mcp_session=None) -> None:
    """Handles a new client connection."""
    session_id = str(id(websocket))
    session = create_session(session_id)
    gemini_session_context = None
    gemini_session = None
    mcp_session = None

    try:
        logger.info(f"Starting new client session: {session_id}")
        
        # Create and initialize Gemini session with shared MCP session
        gemini_session_context, mcp_session = await create_gemini_session(shared_mcp_session)
        session.genai_session = gemini_session_context
        session.mcp_session = mcp_session

        # Enter the Gemini session context
        gemini_session = await gemini_session_context.__aenter__()
        session.active_genai_session = gemini_session

        logger.info(f"Gemini session created for {session_id}")

        # Send ready message to client - simplified
        try:
            logger.debug("Sending ready message...")
            await websocket.send('{"ready": true}')
        ...
        try:
            ...
            logger.error(f"Error in message handling for session {session_id}: {e}")
            if (
                "code = 1006" in str(e)
        ...
        # Clean up sessions
        if gemini_session_context:
            try:
                logger.info("Closing Gemini session...")
                await gemini_session_context.__aexit__(None, None, None)
                logger.info("Gemini session closed")
            except Exception as e:
                logger.error(f"Error closing Gemini session: {e}")
        
        if mcp_session and not shared_mcp_session:  # Only close if not shared
            try:
